...
prey_file=prey.sh
platform=`uname`
linux_packages='curl wget traceroute scrot'
...
separator="--------------------------------------------------------------------------------"
  ...
  # now we always install to /usr/share/prey, as it seems it works for everyone
	...
	if [ "$SKIP" == "$YES" ]; then
		...
		if [ "$REPORT_METHOD" == 'http' ]; then
			...
			if [ "$USER_REGISTERED" == "$YES" ]; then
				...
				if [[ -n "$WEB_USERNAME" && -n "$WEB_EMAIL" && -n "$WEB_PASSWORD" ]]; then
						...
						echo -e '\n -- Registering to Prey Control Panel...'
					user_response=`curl -s -d "user[name]=$WEB_USERNAME&user[email]=$WEB_EMAIL&user[password]=$WEB_PASSWORD&user[password_confirmation]=$WEB_PASSWORD" $WEB_SERVICE_URL/users.xml`
					...
					echo -e "\n\n -- Registration succesful! Remember to log in as $WEB_EMAIL in $WEB_SERVICE_URL whenever you want to change your settings or view your reports."
			...
			if [ "$ADD_DEVICE_AUTO" == 'n' ]; then
				...
				hostname=`hostname`
				device_response=`curl -s -d "api_key=$API_KEY&device[title]=$hostname&device[device_type]=Desktop&device[os]=Linux" $WEB_SERVICE_URL/devices.xml`
				...
				if [ -n "$DEVICE_KEY" ]; then
					echo -e " -- Everything ok! Your Device key is $DEVICE_KEY. We'll add it to your config file automagically as well."
				else
					echo -e " !! There was a problem adding this device to your account. If you wrote down the API Key manually please check it and try again."
				fi

			fi

		elif [ "$REPORT_METHOD" == 'email' ]; then
			...
			[yY] )
				...
				CHECK_URL=`echo $URL | sed "s/\//-SLASH-/g"`
			;;
		...
		sed -i -e "s/check_url='.*'/check_url='$CHECK_URL'/" $temp_config_file
		...
		ENCODED_PASS=`echo -n "$SMTP_PASS" | openssl enc -base64`
		sed -i -e "s/smtp_password='.*'/smtp_password='$ENCODED_PASS'/" $temp_config_file
	...
	if [ "$SKIP" != "$YES" ]; then
		...
		sudo mv $temp_config_file $INSTALLPATH/$config_file
		sudo chmod 700 $INSTALLPATH/$config_file # no read access to other users, for security

	fi

	sudo rm -f $INSTALLPATH/$temp_config_file $INSTALLPATH/install.sh 2> /dev/null
